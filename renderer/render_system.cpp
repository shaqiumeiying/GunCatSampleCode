...
void RenderSystem::draw(float elapsed_ms)
{
	...
	//glClearColor(GLfloat(128 / 255), GLfloat(128 / 255), GLfloat(128 / 255), 1.0);
	...
	{
		if (!registry.motions.has(entity)) // to optimize
			continue;
		...
		drawTexturedMesh(entity, pv_matrix, frame_current, frame_width, elapsed_ms);
	}
	//Weapon &weapon = registry.weapons.get(weapon_system.equipped_weapon);
	//if (weapon.reload_active)
	//{
	//	float progress = 1.0 - weapon.reload_time_ms / weapon.reload_timer;
	//	double cursor_x, cursor_y;
	//	glfwGetCursorPos(window, &cursor_x, &cursor_y);
	//	vec2 cursor_pos = vec2(cursor_x, cursor_y);
	//	GLuint reload_program = effects[(GLuint)EFFECT_ASSET_ID::RELOAD];
